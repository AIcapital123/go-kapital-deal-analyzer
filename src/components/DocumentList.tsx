...
const years = Array.from({ length: 8 }, (_, index) => new Date().getFullYear() - index + 1);
...
export const DocumentList = ({ documents, onCategoryChange, onPeriodChange, onRemove }: {
  ...
  return (
    <div className="mt-5 overflow-hidden rounded-xl border border-[#DDE3E8] bg-white">
      <div className="border-b border-[#E7EBEF] px-4 py-3"><h3 className="text-sm font-extrabold text-[#16365D]">Selected files <span className="ml-1 text-[#667085]">({documents.length})</span></h3></div>
            ...
            <div className="space-y-2">
              ...
              {document.category === "Bank statement" && (
                <div className="grid grid-cols-2 gap-2">
                  <div><label htmlFor={`month-${document.id}`} className="mb-1 block text-[11px] font-bold text-[#667085]">Statement month</label><Select value={document.statementMonth ? String(document.statementMonth) : undefined} onValueChange={(value) => onPeriodChange(document.id, Number(value), document.statementYear)}><SelectTrigger id={`month-${document.id}`} className="h-9 rounded-lg border-[#DDE3E8] text-xs"><SelectValue placeholder="Month" /></SelectTrigger><SelectContent>{statementMonths.map((month, index) => <SelectItem key={month} value={String(index + 1)}>{month}</SelectItem>)}</SelectContent></Select></div>
                  <div><label htmlFor={`year-${document.id}`} className="mb-1 block text-[11px] font-bold text-[#667085]">Statement year</label><Select value={document.statementYear ? String(document.statementYear) : undefined} onValueChange={(value) => onPeriodChange(document.id, document.statementMonth, Number(value))}><SelectTrigger id={`year-${document.id}`} className="h-9 rounded-lg border-[#DDE3E8] text-xs"><SelectValue placeholder="Year" /></SelectTrigger><SelectContent>{years.map((year) => <SelectItem key={year} value={String(year)}>{year}</SelectItem>)}</SelectContent></Select></div>
                </div>
              )}
            </div>
