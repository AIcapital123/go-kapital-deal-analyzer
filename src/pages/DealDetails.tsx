  ...
  return (
    <>
      ...
      <PageHeader
        ...
        actions={<><StatusBadge status={deal.status} />{deal.entryMode === "automated" && <span className="inline-flex items-center gap-1 rounded-full bg-[#EEF9EE] px-3 py-1 text-xs font-bold text-[#3A9738]"><Sparkles className="h-3.5 w-3.5" />AI Underwriter</span>}<Button variant="outline" onClick={runAnalysis} disabled={running} className="rounded-lg border-[#BFC9D2] font-bold text-[#16365D]">{running ? <RefreshCw className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />}{running ? "Loading Sample…" : "Run Sample Analysis Again"}</Button><Button onClick={() => toast.info("Deal editing will be enabled with the connected deal database.")} className="rounded-lg bg-[#16365D] font-bold hover:bg-[#102B4B]"><Edit3 className="mr-2 h-4 w-4" />Edit Deal</Button></>}
      />
